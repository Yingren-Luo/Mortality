...
ui <- navbarPage(
  title = "Mortality trend for 2000-2019",
  tabPanel(
    "Maternal mortality",
    fluidRow(
      column(
...
server <- function(input, output) {
  ...
  output$MatMor <- renderPlotly({
      plot_ly(data = selected(), x = ~year, y = ~MatMor, color = ~country_name) %>%
      add_lines(linetype = ~armconf1) %>%
      layout(title = "Longitudinal plot", xaxis = list(title = 'Year'),
             yaxis = list(title = 'Mortality',type = 'log'))
  })
  
  output$InfMor <- renderPlotly({
    plot_ly(data = selected(), x = ~year, y = ~InfMor, color = ~country_name) %>%
      add_lines(linetype = ~armconf1) %>%
      layout(title = "Longitudinal plot", xaxis = list(title = 'Year'),
             yaxis = list(title = 'Mortality',type = 'log'))
  })
  
  output$NeoMor <- renderPlotly({
    plot_ly(data = selected(), x = ~year, y = ~NeoMor, color = ~country_name) %>%
      add_lines(linetype = ~armconf1) %>%
      layout(title = "Longitudinal plot", xaxis = list(title = 'Year'),
             yaxis = list(title = 'Mortality',type = 'log'))
  })
  
  output$Un5Mor <- renderPlotly({
    plot_ly(data = selected(), x = ~year, y = ~Un5Mor, color = ~country_name) %>%
      add_lines(linetype = ~armconf1) %>%
      layout(title = "Longitudinal plot", xaxis = list(title = 'Year'),
             yaxis = list(title = 'Mortality',type = 'log'))
  })
